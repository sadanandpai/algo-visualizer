import { CellPosition } from '../helpers/interfaces';

export function* getNumberOfIslandsAsync(grid: boolean[][]) {
  let gridClone = grid.map(row => [...row]); // clone the grid to avoid mutation
  const vistedGridCells = Array.from(new Array(grid.length), () => new Array(grid[0].length));

  for (let i = 0; i < gridClone.length; i++) {
    for (let j = 0; j < gridClone[0].length; j++) {
      if (gridClone[i][j]) {
        vistedGridCells[i][j] = true;
        yield* countIslandsAsync(gridClone, vistedGridCells, i, j, gridClone.length, gridClone[0].length);
